...
- A Joint Lab member may be an external collaborator and does not need to be a
  ...
  empty and use `home_display_group` when they should also appear in a Home
  group.
...
- Home shows the affiliation link for Joint Lab members by default. Set
  `hide_home_joint_lab_affiliation: true` for a lab-level leader whose Main Lab
  role already establishes the relationship, such as Taku Komura.
...
- `/mirod-lab/` is the only page with the full Joint Lab section and its empty
  Publications area.
- Home merges profiles with `home_display_group` into the corresponding
  `content/home/people.md` groups. Profiles with `joint_lab_role` receive the
  `Affiliated with Mirod Joint Lab` link below their name by default; the link
  targets `/mirod-lab/`. Profiles with
  `hide_home_joint_lab_affiliation: true` are intentionally omitted from this
  Home-only badge.
...
4. Check Home and `/mirod-lab/` in light and dark themes at desktop and mobile
   widths. Confirm the People/About page has no standalone Joint Lab block.
