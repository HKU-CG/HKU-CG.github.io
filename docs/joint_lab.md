...
- A Joint Lab member may be an external collaborator and does not need to be a
  ...
  not change `user_groups` or move the person to an alumni group. Use
  `home_display_group` when that person should also appear in a Home group.
...
- Home merges profiles with `home_display_group` into the corresponding
  ...
  `Affiliated with Mirod Joint Lab` link below their name; the link targets
  `/mirod-lab/`.
